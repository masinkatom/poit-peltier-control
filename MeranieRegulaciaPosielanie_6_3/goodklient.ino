...
// ==========================================
// PREPÍNAČ: SIMULÁCIA VS REÁLNY SENZOR
// ==========================================
// Ak je tento riadok odkomentovaný -> kód generuje náhodné čísla (15.0 až 30.0 °C)
// Ak tento riadok zakomentuješ (pridáš //) -> kód začne reálne čítať DS18B20 senzor
// #define SIMULACIA_SENZORA

// ==========================================
// HARDVÉR SETUP (SENZOR DS18B20 & PELTIER)
// ==========================================
#define ONE_WIRE_BUS 32
#define PELTIER_PIN 15
#define PWM_FREQ 10
...
const char* serverUrl = "http://192.168.1.106:5000/api/telemetria"; 
...
// ==========================================
String aktualny_stav = "CLOSE"; // Stavy z webu: OPEN, START, STOP, CLOSE
...
void setup() {
  ...
  espServer.begin();
  Serial.println("ESP32 server spustený and pripravený na riadenie...");
}
...
void spustiRegulaciuAOdosliData() {
  ...
  // 1. VÝBER MEDZI SIMULÁCIOU A REÁLNYM SENZOROM
  #ifdef SIMULACIA_SENZORA
    // Generovanie náhodného desatinného čísla medzi 15.00 a 30.00 °C
    temperature = 15.0 + ((float)random(0, 1501) / 100.0);
  #else
    // REÁLNE ČÍTANIE HODNÔT ZO SENZORA DS18B20
  ...
  // 2. PI REGULÁTOR - ČISTÁ MATEMATIKA PRE CHLADENIE
  if (aktualny_stav == "START") {
    
    // CHLADENIE: Odchýlka = Aktuálna - Želaná
    // Ak je teplota vyššia ako setpoint -> error je kladný -> PWM rastie
    // Ak je teplota nižšia ako setpoint -> error je záporný -> PWM klesá k nule
    error = temperature - cielova_teplota;
    integral += error;
    ...
    // Striktné hardvérové limity PWM (oreže záporný výkon na čistú 0)
